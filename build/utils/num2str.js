...
var minus;
var k = 0, i, j;
var N = ["", "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять",
...
function num2str(money) {
    ...
    if (money.indexOf(".") != -1) {
        ...
        rub = money;
    var ru = propis(price = rub, R);
    var ko = propis(price = kop, K);
    var res;
    ko != "" ? res = ru + " " + ko : res = ru;
    ru == "Ноль " + R[0] && ko != "" ? res = ko : 0;
    // kop == 0 ? res += " ноль " + K[0] : 0;
    var result = (minus + res).substr(0, 1).toUpperCase() + (minus + res).substr(1);
    return result;
}
...
function propis(price, D) {
    ...
    for (i = 0; i < price.length; i += 3) {
        ...
        if (price.substr(price.length - i - 3, 3) == "000" && edinicy == " " + M[0][i / 3 + 3])
            edinicy = "";
